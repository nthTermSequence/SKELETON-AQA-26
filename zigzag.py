...
def Main():
    ...
    while Choice != "9":
        ...
        if Choice == "1":
            ...
            End = ThisSimulation.AdvanceStage(1)
            ...
            End = ThisSimulation.AdvanceStage(NumberOfStages)
...
def GetCellReference(Rows, Columns):
    print()
    Row = input("Enter row number: ")
    while not Row.isdecimal() or not (1 <= int(Row) <= Rows):
    ...
    while not Column.isdecimal() or not (1 <= int(Column) <= Columns):
...
def DisplaySimulationChoices():
    Sims = [[1, 5, 5, 500, 3, 5, 1000, 50],
            [1, 5, 5, 500, 3, 5, 1000, 100],
            [1, 10, 10, 500, 3, 9, 1000, 25],
            [2, 10, 10, 500, 3, 6, 1000, 25]]
    for i in range(len(Sims)):
        print("Simulation %i:\nStartingNumberOfNests: %i\nNumberOfRows: %i\nNumberOfColumns: %i\nStartingFoodInNest: %i\nStartingNumberOfFoodCells: %i\nStartingAntsInNest: %i\nNewPheromoneStrength: %i\nPheromoneDecay: %i" % tuple((i + 1, *Sims[0])))
        print("\n")
        
def GetSimChoice():
    Choice = input("Enter simulation number: ")
    while not Choice.isdecimal() or not (1 <= int(Choice) <= 4):
        if not Choice.isdecimal():
            print("Not valid format.")
        else:
            print("Not in range.")
...
class Simulation():
    def __init__(self, SimulationParameters):
        ...
        for Count in range(1, self._StartingNumberOfFoodCells + 1):
            Row = 2
            Column = 4
            while (Row == 2 and Column == 4) or self._Grid[self.__GetIndex(Row, Column)].GetAmountOfFood() > 0:
                Row = random.randint(1, self._NumberOfRows)
                Column = random.randint(1, self._NumberOfColumns)
        ...
        for Count in range(1, NumberOfStages + 1):
            for C in self._Grid:
                if C.GetAmountOfFood() > 0:
                    C.UpdateFoodInCell(-C.GetAmountOfFood() // 10)
            PheromonesToDelete = []
            for P in self._Pheromones:
    ...
    def GetEndReason(self):
        Reasons = []
        if len(self._Ants) == 0:
            Reasons.append("all the ants passed away")
